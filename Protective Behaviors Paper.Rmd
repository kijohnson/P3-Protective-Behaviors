...
# Align survey date with MO HOSP files week beginning dates in 
```{r}
...
# limit to Black and White for ttest
Baseline_BlWh <- Baseline %>%
  filter(race_cat %in% c("White", "Black or African American"))

# testing significance of difference of baseline worry score between Black and White participants
t.test(Baseline_BlWh$worry_bl_1 ~ Baseline_BlWh$race_cat,
       var.equal = TRUE)
...
wilcox.test(formula = Baseline_BlWh$worry_bl_1 ~ Baseline_BlWh$race_cat,
            paired = FALSE)
# not significant p=0.235
```

# Figure 2. Trends in mean COVID-19 worry score by racial group.
```{r}
